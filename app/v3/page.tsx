    ...
    Modal.confirm({
      ...
      onOk: () => {
        ...
        (async () => {
          ...
          setSubmitting(true);
          try {
            // —— 新流程：先创建任务（≤1s 返回 task_id）→ 经 Vercel 服务端转发上传文件并激活任务 ——
            // 文件传输走 Vercel CDN，比浏览器直连 Supabase Storage（国内网络极慢）稳定

            // 1. 创建任务（不传文件，仅 rule_id 元数据）
            ...
            try {
              ...
              if (uploadData.ok || uploadData.task_id) {
                hideLoading();
